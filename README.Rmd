...
high-dimensional fixed effects. It uses a Huber loss function and iteratively 
reweighted least squares (IRLS) and is inspired by 
...
numerical behavior may and probably will change. Use with care and 
...
(Stata's `robreg`) and that absorb fixed effects accross two dimensions (Stata's
`robtwfe`). Summary statistics of the relative differences of `ferols`'s 
...
based on `{r} unique(bmark_stats$runs)` runs on independent randome samples 
