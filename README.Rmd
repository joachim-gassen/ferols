...
comp_stats <- readRDS("utils/comp_stats.rds")
bmark_est_res <- readRDS("utils/bmark_est_res.rds")
```

The R package `ferols`^[Technically, the name `ferols` is misleading, as the package uses iteratively reweighted least squares and not plain OLS to derive its estimates. But then again, `ferreg()` sounds like a rodent to me and I liked the analogy of the name with `fixest::feols()`. Alternative name suggestions are always welcome ;-).] implements a robust M-Estimator for linear models with 
...
> Robust Inference in Archival Research, Journal of Accounting and Economics,
> forthcoming, https://doi.org/10.1016/j.jacceco.2026.101895.
...
The average processing times in seconds for various fixed effect structures can 
be compared by the following table. Notice the speed gain relative to 
`MASS::rlm()`, which has to estimate the coefficients and SEs for all fixed 
effects. Missing values are caused by failure of the respective package to 
yield an estimate.

```{r TabBenchmark, echo=FALSE}
...
colnames(df) <- c(
  "N [units]", "N [time]", "Runs", "R `ferols`", "R `rlm`", 
  "Stata `robhdfe`", "Stata `robreg`"
)
rownames(df) <- NULL
kable(df, align = "r", digits = 2) 
```
...
identical within numerical precision to the ones created by packages that do 
not aborb fixed effects (`MASS::rlm()`)^[To mimic the algorithm used by 
...
based on `{r} unique(comp_stats$runs)` runs on independent random samples 
...
```{r TabComparison, echo=FALSE}
df <- comp_stats[1:9,c(1:2, 4:6)]
df[df[,2] == "est", 2] <- "coef"
