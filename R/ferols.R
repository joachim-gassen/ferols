...
#' and the actual estimation steps are relegated to [fixest::feols()].
#' Standard errors can be clustered via multiple levels and alternative
#' standard error estimates provided via [fixest] are supported using a Huber 
#' (psi/phi) modified sandwich estimator.
#'
...
#' @param vcov Variance–covariance specification as required by 
#'  [fixest::feols()]. Either \code{"iid"}, 
#'  \code{"hetero"}, \code{"cluster"}, \code{"twoway"},
#'  \code{"NW"}, \code{"DK"}, \code{"conley"},  or a formula 
...
#' 
#' It supports various variance–covariance matrices estimates. 
#' For "iid" standard errors, the method mirrors
#' the approach of [MASS::rlm()]. All other standard error types are 
#' relegated to [fixest::vcov.fixest()] after the bread
#' part of the sandwich estimator is adjusted as discussed in Appendix A.2 of 
#' [Gassen and Veenman (2026)](https://doi.org/10.1016/j.jacceco.2026.101895).
#' This has been verified to yield consistent standard errors for (multiway)
#' clustered SEs but should also work for other SEs (e.g., Newey-West, 
#' Discroll-Kray, and Conley) that follow the sandwich estimator structure. 
#' Use with care for these and feel free to open an issue on GitHub
#' if you sense any inconsistencies.
#' 
...
#'
#' @description
#' Computes variance–covariance matrices for estimates generated by
#' [ferols]. For "iid" standard errors, the method mirrors
#' the approach of [MASS::rlm()]. All other error types are 
#' relegated to [fixest::vcov.fixest()] after the bread
#' part of the sandwich estimator is adjusted as discussed in Appendix A.2 of 
#' [Gassen and Veenman (2026)](https://doi.org/10.1016/j.jacceco.2026.101895).
#' This has been verified to yield consistent standard errors for (multiway)
...
#' @param vcov Variance–covariance specification. See 
#' [fixest::vcov.fixest()] for details. Defaults to heteroskedasticity-robust 
